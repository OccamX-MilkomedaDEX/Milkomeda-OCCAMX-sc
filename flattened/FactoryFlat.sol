...
contract ERC20Permit is IERC20Permit {
    ...
    string public override constant name = 'Liquidity Token';
    string public override constant symbol = 'LT';
...
// File contracts/interfaces/ICallee.sol
...
contract Pair is ERC20Permit {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) _mint(feeTo, liquidity); // this changes the totalSupply
                }
            }
        } else if (_kLast != 0) {
            kLast = 0;
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1); // totalSupply can be changed here
        uint _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can update in _mintFee
